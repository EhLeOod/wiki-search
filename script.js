...
$(document).ready(function(){
  ...
  $('#input').keypress(function (e) {
    ...
    if (e.which == 13) {
      if (search == '') {
        $('#errorBlank').text('Oopsie-poo. You must enter something in the search box to get Wikipedia search results! Or live in ignorance forever...');
        $('#errorBlank').show();
        $('#errorSearch').hide();
      ...
      $.ajax({
        ...
        success: function(response) {
          if (response.query.searchinfo.totalhits === 0) {
            $('#errorSearch').text('Your search for  \" ' + search + '\"  did not return any results. Try checking your spelling, or using a more general or different search term! Don\'t ever give up... In Wiki-ing, Gooogle-ing, or life.');
            $('#errorSearch').show();
            $('#errorBlank').hide();   
            } else if (search == '') {
              $('#errorBlank').show();
